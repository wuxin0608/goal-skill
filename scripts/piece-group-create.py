...
def run(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not title:
        raise ValueError("缺少 title（文案分组名）")
    body: Dict[str, Any] = {
        "project_id": p["project_id"],
        "title": title,
        "sort_order": int(p.get("sort_order") or p.get("sortOrder") or 0),
    }
    payload = request_api("POST", "/v1/project_piece_groups/create", body=body)
...
def main(argv: Optional[list] = None) -> int:
    ...
    if not argv:
        print('用法: python piece-group-create.py \'{"title":"默认文案","sort_order":0}\'', file=sys.stderr)
        return 1
    try:
